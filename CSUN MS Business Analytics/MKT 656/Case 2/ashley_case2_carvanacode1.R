...
logit_result <- glm(formula = IsBadBuy ~  lnVehBCost + Make_Chevrolet + Model_PTCRUISER + State_TX + State_FL +
                      as.factor(WheelType) + MMRCurrentRetailAveragePrice + MMRCurrentAuctionAveragePrice + MMRAcquisitionRetailAveragePrice + 
                      MMRAcquisitionAuctionAveragePrice, data = logit, family = "binomial")
